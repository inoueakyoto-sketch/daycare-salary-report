...
# apl/nm/ot/pt/st/cp の各カテゴリページには、訪問看護・病院・介護施設・
# 児童発達支援(0-6歳専用)など全く別種の求人も大量に混在するため、
# 「放課後等デイサービス」であることを示す語が含まれない求人は採用しない(ポジティブフィルタ)。
INCLUDE_HINT = ["放課後等デイサービス", "放課後デイサービス", "放デイ"]
...
def parse_jobs(html: str, city: str, role: str) -> list[dict]:
    ...
    for i, m in enumerate(matches):
        ...
        # 放課後等デイサービスの求人であることを必須とする(施設名・カード本文いずれかに
        # 「放課後等デイサービス」等の語が無ければ、他業種(訪問看護・病院・児童発達支援単独等)
        # の求人とみなして除外する)
        haystack = facility + " " + block_text
        if not any(k in haystack for k in INCLUDE_HINT):
            continue
